/*
 * 合成 Apple /clls/wloc 介面的回應，解 ARPC 封包，改 WiFi 熱點和基地台座標，
 * 再依 Apple 的格式送回去給系統。
 *
 * 主要流程：
 *   ARPC 封包 → protobuf 解碼字串 → 替換 Location 子訊息的座標／精度／運動狀態
 *   → protobuf 重新打包 → 依原格式（ARPC / marker / synthetic）封裝
 */
(function () {
  "use strict";
  ...
  function parseFields(bytes) {
    ...
    while (offset < bytes.length) {
      ...
      fields.push({
        fieldNumber: fieldNumber, wireType: wireType, keyStart: keyStart, valueStart: valueStart,
        valueEnd: valueEnd, end: valueEnd, raw: bytes.slice(keyStart, valueEnd),
  ...
  function locationSummary(locationPayload) {
    ...
    } catch (err) { return "<parse-failed:" + err.message + ">"; }
  }
  ...
  function spoofArpcRequest(requestBytes, configInput) {
    ...
    return {
      ...
      payload: patched.payload, wifiCount: patched.wifiCount,
      cellCount: patched.cellCount, arpc: arpc
    };
  }
  ...
  function spoofAppleResponse(responseBytes, configInput) {
    ...
    return {
      response: response, payload: patched.payload, wifiCount: patched.wifiCount,
      cellCount: patched.cellCount, kind: extraction.kind,
  ...
  function parseArgumentString(argument) {
    ...
    var tailKeys = [
      "debug","mode","enabled","latitude","longitude","altitude","address",
      "configHost","configToken","horizontalAccuracy","verticalAccuracy","randomRadius",
      "unknownValue4","motionActivityType","motionActivityConfidence","failOpen",
      "dumpRaw","dumpHeaders","prepareHeaders","rawLimit"
    ];
    ...
    if (configUrlIdx >= 0) {
      ...
      try { result.configUrl = decodeURIComponent(configUrlValue); }
      catch (err) { result.configUrl = configUrlValue; }
    ...
    for (var j = 0; j < pairs.length; j += 1) {
      ...
      try { result[decodeURIComponent(key)] = decodeURIComponent(value); }
      catch (err2) { result[key] = value; }
    }
    return result;
  }
  ...
  function enrichArgsFromPluginStore(args) {
    var keys = ["enabled","latitude","longitude","altitude","horizontalAccuracy",
      "verticalAccuracy","randomRadius","address","configHost","configToken","configUrl","debug"];
    var i;
    args = args || {};
  ...
  function readScriptArguments() {
    ...
    if (typeof $argument !== "undefined" && $argument != null) {
      ...
      } else { out = parseArgumentString(String($argument)); }
    }
  ...
  function logScriptArguments(debug) {
    ...
    var raw = typeof $argument === "undefined" || $argument == null ? "<none>"
      : typeof $argument === "object" ? JSON.stringify($argument) : String($argument);
    console.log("Location spoofer $argument raw: " + raw);
    console.log("Location spoofer args parsed: lat=" + args.latitude + ", lng=" + args.longitude + ", configUrl=" + (resolveConfigUrl(args) || "<none>"));
  }
  ...
  function writeGeocodeCache(entry) {
    ...
    try { $persistentStore.write("location_spoofer_geocode", JSON.stringify(entry)); }
    catch (err) {}
  }

  function fetchElevation(lat, lng, callback) {
    if (typeof $httpClient === "undefined" || !$httpClient.get) { callback(null); return; }
    var url = "https://api.open-meteo.com/v1/elevation?latitude=" + encodeURIComponent(String(lat)) + "&longitude=" + encodeURIComponent(String(lng));
    $httpClient.get({ url: url, timeout: 4000 }, function (error, response, body) {
      if (error || !body) { callback(null); return; }
      try {
        var data = JSON.parse(body);
        if (data && data.elevation && data.elevation.length) { callback(Math.round(Number(data.elevation[0]))); return; }
      } catch (err) {}
      callback(null);
    });
  }
  ...
  function geocodeAddress(address, debug, callback) {
    ...
    if (cached && cached.address === query && Number.isFinite(Number(cached.latitude)) && Number.isFinite(Number(cached.longitude))) {
      if (debug) { console.log("Location spoofer geocode cache hit: " + query + " -> " + cached.latitude + "," + cached.longitude); }
      callback(cached);
      return;
    }
    if (typeof $httpClient === "undefined" || !$httpClient.get) {
      if (debug) { console.log("Location spoofer geocode skipped: $httpClient unavailable"); }
      callback(null);
      return;
    }
    var url = "https://nominatim.openstreetmap.org/search?format=json&limit=1&addressdetails=0&q=" + encodeURIComponent(query);
    $httpClient.get({ url: url, timeout: 8000, headers: { "User-Agent": "ios-location-spoofer/1.0 (Loon plugin)" } }, function (error, response, body) {
      if (error || !body) {
        if (debug) { console.log("Location spoofer geocode failed: " + (error || "empty body")); }
        callback(null);
        return;
      }
      try {
        ...
        if (!results || !results.length) {
          if (debug) { console.log("Location spoofer geocode no result for: " + query); }
          callback(null);
          return;
        }
        var hit = results[0];
        ...
        if (!Number.isFinite(lat) || !Number.isFinite(lng)) { callback(null); return; }
        var entry = { address: query, latitude: lat, longitude: lng, displayName: hit.display_name || query };
        fetchElevation(lat, lng, function (altitude) {
          if (altitude != null) { entry.altitude = altitude; }
          writeGeocodeCache(entry);
          if (debug) { console.log("Location spoofer geocode resolved: " + query + " -> " + lat + "," + lng + (altitude != null ? ", alt=" + altitude : "")); }
          callback(entry);
        });
      } catch (err) {
        if (debug) { console.log("Location spoofer geocode parse failed: " + err.message); }
        callback(null);
      }
    });
  }
  ...
  function configFromArgs(args) {
    var cfg = {};
    var scalarKeys = ["enabled","mode","latitude","longitude","address","horizontalAccuracy",
      "verticalAccuracy","randomRadius","altitude","unknownValue4","motionActivityType",
      "motionActivityConfidence","failOpen","debug","dumpRaw","dumpHeaders","prepareHeaders","rawLimit"];
  ...
  function readRemoteConfigCache(url) {
    ...
    try {
      ...
      if (Date.now() - entry.ts > 300000) { return null; }
      return entry.data;
    } catch (err) { return null; }
  }
  ...
  function writeRemoteConfigCache(url, data) {
    ...
    try { $persistentStore.write("location_spoofer_remote_cfg", JSON.stringify({ url: url, data: data, ts: Date.now() })); }
    catch (err) {}
  }
  ...
  function fetchRemoteConfig(url, timeout, debug, callback) {
    ...
    $httpClient.get({ url: url, timeout: timeout || 3000 }, function (error, response, body) {
      ...
      try { callback(JSON.parse(body), null); }
  ...
  function refreshRemoteConfigCache(url, debug) {
    fetchRemoteConfig(url, 5000, debug, function (data, err) {
      if (data) { writeRemoteConfigCache(url, data); return; }
      if (debug) { console.log("Location spoofer remote config refresh failed: " + err); }
    });
  }
  ...
  function applyAddressFromCache(cfg, address, debug) {
    ...
    if (cached && cached.address === address && Number.isFinite(Number(cached.latitude)) && Number.isFinite(Number(cached.longitude))) {
      ...
      if (cached.altitude != null) { cfg.altitude = cached.altitude; }
      if (debug) { console.log("Location spoofer geocode cache hit: " + address); }
      return;
    }
    if (debug) { console.log("Location spoofer geocode cache miss: " + address + " (use manual lat/lng until cron refreshes)"); }
  }
  ...
  function loadRuntimeConfigSync() {
    ...
    if (configUrl) {
      ...
      if (remoteCfg) {
        ...
        if (debug) { console.log("Location spoofer remote config cache hit -> " + remoteCfg.latitude + "," + remoteCfg.longitude); }
      }
    }
  ...
  function loadRuntimeConfig(callback) {
    ...
    function finish() {
      try { callback(normalizeConfig(cfg)); }
      catch (err) {
        if (debug) { console.log("Location spoofer config invalid: " + err.message + " | cfg lat/lng=" + cfg.latitude + "," + cfg.longitude); }
    ...
    if (readRemoteConfigCache(configUrl)) { refreshRemoteConfigCache(configUrl, debug); finish(); return; }
    if (debug) { console.log("Location spoofer remote config fetching: " + configUrl); }
    fetchRemoteConfig(configUrl, 3000, debug, function (data, err) {
      if (data) {
        writeRemoteConfigCache(configUrl, data);
        cfg = mergeConfig(cfg, data);
        if (debug) { console.log("Location spoofer remote config loaded -> " + data.latitude + "," + data.longitude); }
      } else if (debug) { console.log("Location spoofer remote config fetch failed: " + err + " (using manual lat/lng)"); }
      finish();
    });
  }
  ...
  function runMaintenanceCron() {
    ...
    var pending = 0;
    function maybeDone() { pending -= 1; if (pending <= 0) { $done({}); } }
    var configUrl = resolveConfigUrl(args);
    if (configUrl) {
      pending += 1;
      fetchRemoteConfig(configUrl, 8000, debug, function (data, err) {
        if (data) {
          writeRemoteConfigCache(configUrl, data);
          if (debug) { console.log("Location spoofer config cron cached -> " + data.latitude + "," + data.longitude); }
        } else if (debug) { console.log("Location spoofer config cron failed: " + err); }
        maybeDone();
      });
    }
    var address = String(args.address || "").trim();
    if (address) {
      pending += 1;
      geocodeAddress(address, debug, function () { maybeDone(); });
    }
    if (pending === 0) { $done({}); }
  }

  function runGeocodeCron() { runMaintenanceCron(); }
  ...
  function setHeader(headers, name, value) {
    ...
    for (var key in headers) {
      if (Object.prototype.hasOwnProperty.call(headers, key) && key.toLowerCase() === lower) { existingKey = key; break; }
    }
    headers[existingKey || name] = value;
    return headers;
  }

  function prepareRequestHeaders(headers) { return setHeader(headers || {}, "Accept-Encoding", "identity"); }
  ...
  function decompressBody(body, contentEncoding) {
    ...
    try {
      ...
      if (typeof console !== "undefined") { console.log("Location spoofer decompress failed (" + enc + "): " + err.message); }
    }
    return body;
  }
  ...
  function prepareResponseBodySync(config) {
    ...
    if (isGzipBytes(bytes) || (contentEncoding && String(contentEncoding).toLowerCase().indexOf("gzip") >= 0)) {
      ...
      if (decoded && decoded.length > 2 && !isGzipBytes(decoded)) {
        ...
        if (config.debug) { console.log("Location spoofer decompressed body: " + bytes.length + " -> " + decoded.length + " bytes"); }
        return;
      }
      if (config.debug) { console.log("Location spoofer gzip body still compressed (len=" + bytes.length + "); ensure http-request prepare script is enabled"); }
      return;
    }
    if (contentEncoding) {
  ...
  function fieldHistogram(fields) {
    ...
    var parts = [];
    for (var j = 0; j < order.length; j += 1) { parts.push(order[j] + "x" + counts[order[j]]); }
    return parts.join(",");
  }

  function countFields(fields, fieldNumber) {
    var count = 0;
    for (var i = 0; i < fields.length; i += 1) { if (fields[i].fieldNumber === fieldNumber) { count += 1; } }
    return count;
  }

  function countCellResponseFields(fields) {
    var count = 0;
    for (var i = 0; i < fields.length; i += 1) { if (isCellResponseField(fields[i].fieldNumber)) { count += 1; } }
    return count;
  }
  ...
  function appleWLocPayloadInspect(payload) {
    try {
      var fields = parseFields(payload);
      var parts = [
        "payloadLen=" + payload.length, "fields=" + fieldHistogram(fields),
        "wifi=" + countFields(fields, 2), "cellResp=" + countCellResponseFields(fields),
        "cellReq=" + countFields(fields, 25),
        "hasCounts=" + (countFields(fields, 3) + "/" + countFields(fields, 4)),
        "deviceType=" + countFields(fields, 33), patchedPayloadSummary(payload)
      ];
      return parts.join(", ");
    } catch (err) { return "payload parse failed: " + err.message; }
  }

  function logRawDump(label, bytes, config) {
    if (!config.dumpRaw || !bytes) { return; }
  ...
  function jsonString(value) {
    ...
    catch (err) { return "<json-failed:" + err.message + ">"; }
  }

  function logHttpDump(label, message, config) {
    if (!config.dumpHeaders && !config.dumpRaw) { return; }
    message = message || {};
    ...
    if (config.dumpHeaders) { console.log("Location spoofer raw " + label + " headers: " + jsonString(message.headers || {})); }
  }

  function inspectResponseBytes(bytes, config) {
    if (!bytes) { console.log("Location spoofer inspect response body unavailable"); return; }
    ...
    try {
      ...
      if (directFields) { console.log("Location spoofer inspect response direct fields: " + fieldHistogram(directFields)); }
    }
  }

  function inspectRequestBytes(bytes, config) {
    if (!bytes) { console.log("Location spoofer inspect request body unavailable"); return; }
    ...
    try {
      ...
      var directFields = tryParseFields(bytes);
      if (directFields) { console.log("Location spoofer inspect request direct fields: " + fieldHistogram(directFields)); }
    }
  }
  ...
  function doneInspect(config, hasResponse) {
    if (hasResponse) {
      ...
      if (config.prepareHeaders) { donePreparedRequestPassThrough(); return; }
    }
    donePassThrough();
  }
  ...
  function continueResponseRewrite(config) {
    ...
    if (!responseBody || responseBody.length < 2) {
      if (config.debug) {
        console.log("Location spoofer response body too short: " + (responseBody ? responseBody.length : 0) + " bytes, head=" + (responseBody ? hexPreview(responseBody) : "<none>"));
      }
      donePassThrough();
      return;
    }
    if (config.debug) {
      ...
      if (isLoonRuntime()) { console.log("Location spoofer runtime: Loon"); }
    }
  ...
  function runShadowrocket() {
    ...
    if (!hasRequest && !hasResponse) { runMaintenanceCron(); return; }
    if (hasRequest && !hasResponse) {
      var prepArgs = readScriptArguments();
      if (parseBoolean(prepArgs.debug, false)) { console.log("Location spoofer prepare -> Accept-Encoding: identity"); }
    ...
    loadRuntimeConfig(function (config) {
      try {
        ...
        if (config.mode === "inspect") { doneInspect(config, hasResponse); return; }
        if (hasResponse) {
          if (config.debug) {
            console.log("Location spoofer intercept -> lat=" + config.latitude + ", lng=" + config.longitude + ", url=" + (($request && $request.url) || "<none>"));
          }
          if (config.mode === "probe") { doneResponseProbe(config); return; }
        ...
        if (config.debug) { console.log("Location spoofer request mode body length: " + (requestBody ? requestBody.length : 0)); }
        if (!requestBody) {
          if (config.debug) { console.log("Location spoofer request body unavailable"); }
          donePassThrough();
          return;
        }
        if (requestBody.length < 2) {
          if (config.debug) { console.log("Location spoofer request body too short: " + requestBody.length + " bytes, head=" + hexPreview(requestBody)); }
          donePassThrough();
          return;
        }
  ...
  var api = {
    DEFAULT_CONFIG: DEFAULT_CONFIG, APPLE_WLOC_PREFIX: APPLE_WLOC_PREFIX,
    APPLE_WLOC_MARKER: APPLE_WLOC_MARKER, bodyToBytes: bodyToBytes,
    messageBodyToBytes: messageBodyToBytes, hexPreview: hexPreview,
    bytesToBinaryString: bytesToBinaryString, bytesToBase64: bytesToBase64,
    binaryStringToBytes: binaryStringToBytes, concatBytes: concatBytes,
    readUInt16BE: readUInt16BE, writeUInt16BE: writeUInt16BE,
    encodeVarintUnsigned: encodeVarintUnsigned, encodeVarintSignedInt64: encodeVarintSignedInt64,
    decodeVarint: decodeVarint, makeVarintField: makeVarintField,
    makeLengthDelimitedField: makeLengthDelimitedField, parseFields: parseFields,
    tryParseFields: tryParseFields, firstFieldByNumber: firstFieldByNumber,
    locationSummary: locationSummary, patchedPayloadSummary: patchedPayloadSummary,
    coordToInt: coordToInt, normalizeConfig: normalizeConfig, patchLocation: patchLocation,
    patchWifiDevice: patchWifiDevice, patchCellTower: patchCellTower,
    patchAppleWLocPayload: patchAppleWLocPayload, parseArpc: parseArpc,
    serializeArpc: serializeArpc, buildAppleWLocResponse: buildAppleWLocResponse,
    extractAppleWLocPayload: extractAppleWLocPayload, spoofArpcRequest: spoofArpcRequest,
    spoofAppleResponse: spoofAppleResponse, parseArgumentString: parseArgumentString,
    readScriptArguments: readScriptArguments, geocodeAddress: geocodeAddress,
